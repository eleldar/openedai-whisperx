...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        help="The model to use for transcription. Ex. distil-whisper/distil-medium.en",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store",
        default="auto",
        help="Set the torch data type for processing (float32, float16, bfloat16)",
    )
...
if __name__ == "__main__":
    ...
    if args.dtype == "auto":
        ...
        dtype = (
            torch.bfloat16 if args.dtype == "bfloat16" else torch.float16 if args.dtype == "float16" else torch.float32
        )
    ...
    pipe = pipeline(
        "automatic-speech-recognition", model=args.model, device=device, chunk_length_s=30, torch_dtype=dtype
    )
    if args.preload:
        sys.exit(0)

    app.register_model(args.model, "large-v3")
